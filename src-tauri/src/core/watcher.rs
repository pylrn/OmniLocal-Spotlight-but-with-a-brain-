...
use crate::workers::embedding::EmbedCommand;
                        ...
                        for (_path, change) in to_process {
                            ...
                            match &change {
                                ...
                                FileChange::Deleted(path) => {
                                    let db_lock = db.lock().unwrap();
                                    match scanner::remove_single_file(&db_lock, path) {
                                        Ok(true) => {
                                            info!("Removed document: {:?}", path);
                                            if let Some(ref handle) = event_sender {
                                                let _ = handle.emit(
                                                    "index-progress",
                                                    serde_json::json!({
                                        ...
                                        Ok(false) => debug!("Deleted file not in database: {:?}", path),
                                        Err(error) => warn!("Failed to remove document {:?}: {}", path, error),
                                    }
                                }
                            }
                        }

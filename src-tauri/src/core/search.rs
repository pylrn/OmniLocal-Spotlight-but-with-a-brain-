...
use crate::core::db::{self, SearchHit};
...
pub fn vector_search(
    ...
    let rows = stmt.query_map(rusqlite::params![query_embedding.as_bytes(), limit as i64], |row| {
    ...
    for row in rows {
        hits.push(row.map_err(|e| format!("Error reading vector result: {}", e))?);
    }

...
pub fn mark_chunk_embedding_success(
    ...
    let embedding_json =
        serde_json::to_string(embedding).map_err(|e| rusqlite::Error::ToSqlConversionFailure(e.into()))?;
    ...
    conn.execute(
        ...
        params![chunk_id, provider, model, dimensions, embedding_json],
    )?;
...
pub fn get_chunk_vectors(conn: &Connection) -> Result<Vec<VectorSearchRow>> {
    let mut stmt = conn.prepare(
        "SELECT ce.chunk_id, c.document_id, c.content, c.chunk_index, c.start_line, c.end_line, ce.embedding
